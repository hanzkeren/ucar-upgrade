import type { NextApiRequest, NextApiResponse } from 'next'
import { kvIncr } from '../../utils/rateLimiter'

async function edgeConfigGet<T = unknown>(key: string): Promise<T | null> {
  try {
    const mod = await import('@vercel/edge-config').catch(() => null as any)
    if (mod && typeof (mod as any).get === 'function') {
      const v = await (mod as any).get(key)
      return (v ?? null) as T | null
    }
  } catch {}
  return null
}

async function getToken(): Promise<string | null> {
  const v = await edgeConfigGet<string>('ANALYTICS_API_TOKEN')
  if (typeof v === 'string' && v.length) return v
  const env = (process.env as any)?.ANALYTICS_API_TOKEN
  return typeof env === 'string' ? env : null
}

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') return res.status(405).json({ ok: false })
  const token = await getToken()
  const auth = req.headers['authorization'] || ''
  if (!token || auth !== `Bearer ${token}`) return res.status(401).json({ ok: false })
  try {
    const body = req.body || (await new Promise<any>((r) => {
      let d=''; req.on('data', c=>d+=c); req.on('end', ()=>{ try{ r(JSON.parse(d||'{}')) }catch{ r({}) } })
    }))
    const exp = (typeof body?.exp === 'string') ? body.exp : null
    const scoreNum = Number(body?.score)
    if (!exp || !Number.isFinite(scoreNum)) return res.status(200).json({ ok: true })
    const bucket = Math.max(0, Math.min(9, Math.floor(scoreNum / 10)))
    await kvIncr(`metrics:exp:${exp}:bucket:${bucket}:fraud`, 24*60*60)
    return res.status(200).json({ ok: true })
  } catch {
    return res.status(500).json({ ok: false })
  }
}

